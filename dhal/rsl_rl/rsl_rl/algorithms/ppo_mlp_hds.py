...
class PPO_HDS:
    ...
    def update(self):
        ...
        for (
            ...
            critic_obs_batch,
            actions_batch,
            target_values_batch,target_glide_values_batch, target_push_values_batch, target_reg_values_batch,
            advantages_batch,
            ...
            # mode_latent = [batch, num_mode] -- one hot
            mode_latent, prob = self.actor_critic.NHA(obs_batch)
            NHA_entropy = -(prob * torch.log(prob + 1e-8)).sum(dim=-1)
            NHA_entropy = torch.mean(NHA_entropy)
            loss_list = []
            recon_loss_list = []
            ...
            for i, sub_net in enumerate(self.actor_critic.TsDyn_modules):
                ...
                total_loss, recon_loss, recon_contact_loss, kl_div = sub_net.loss_function(recon_x, obs_future_batch, recon_contact, contact_future_batch, mu, logvar)
                temp_loss = total_loss.unsqueeze(1).unsqueeze(2) # Shape: [batch,1,1]
                loss_list.append(temp_loss) # Shape: [batch,1,1]

                recon_loss = recon_loss.unsqueeze(1).unsqueeze(2) # Shape: [batch,1,1]
                recon_loss_list.append(recon_loss) # Shape: [batch,1,1]

                recon_contact_loss = recon_contact_loss.unsqueeze(1).unsqueeze(2) # Shape: [batch,1,1]
                recon_contact_loss_list.append(recon_contact_loss) # Shape: [batch,1,1]

                kl_div = kl_div.unsqueeze(1).unsqueeze(2) # Shape: [batch,1,1]
                kl_div_list.append(kl_div) # Shape: [batch,1,1]

            loss = torch.cat(loss_list, dim=1)  # Shape: [batch,N_modes,1]
            vae_loss = torch.bmm(mode_latent.unsqueeze(1), loss).squeeze(1) # Shape: [batch,1]
            vae_loss = torch.mean(vae_loss)

            recon_loss = torch.cat(recon_loss_list, dim=1)  # Shape: [batch,N_modes,1]
            recon_loss = torch.bmm(mode_latent.unsqueeze(1), recon_loss).squeeze(1) # Shape: [batch,1]
            recon_loss = torch.mean(recon_loss)

            recon_contact_loss = torch.cat(recon_contact_loss_list, dim=1)  # Shape: [batch,N_modes,1]
            recon_contact_loss = torch.bmm(mode_latent.unsqueeze(1), recon_contact_loss).squeeze(1) # Shape: [batch,1]
            recon_contact_loss = torch.mean(recon_contact_loss)

            kl_div = torch.cat(kl_div_list, dim=1)  # Shape: [batch,N_modes,1]
            kl_div = torch.bmm(mode_latent.unsqueeze(1), kl_div).squeeze(1) # Shape: [batch,1]
            ...
            loss = surrogate_loss + self.value_loss_coef * value_loss  - self.entropy_coef * entropy_batch.mean() + vae_loss + self.entropy_coef * NHA_entropy
            ...
            mean_NHA_entropy += NHA_entropy.item()
            mean_recon_loss += recon_loss.item()
            mean_recon_contact_loss += recon_contact_loss.item()
            mean_kl_div += kl_div.item()
        ...
        mean_NHA_entropy /= num_updates
        mean_recon_loss /= num_updates
        mean_recon_contact_loss /= num_updates
        mean_kl_div /= num_updates
        self.storage.clear()

        return mean_value_loss, mean_surrogate_loss, mean_vae_loss, mean_NHA_entropy, mean_recon_loss, mean_recon_contact_loss, mean_kl_div
