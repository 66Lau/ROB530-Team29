...
class NHashCuriosity:
    def __init__(self, cfg, device):
        self.cfg = cfg
        self.device = device
        self.hashnn = myNN(cfg.curiosity.obs_dim, cfg.curiosity.hidden_sizes_hash, cfg.curiosity.pred_dim).to(device)
    ...
    def normalize(self, obs):
        ...
        obs_ = (obs_-self.obs_lb) / (self.obs_ub - self.obs_lb)
        assert obs_.shape[-1] == self.cfg.curiosity.obs_dim
        return obs_
    ...
    def update_curiosity(self, obs:torch.Tensor, shift=0.)-> torch.Tensor:
        ...
        obs_ *= (1 + shift.repeat(1,1))  # expend resolution
        hash_ = self.hashnn(obs_)
        bin_idx = self.bin2int(hash_)
