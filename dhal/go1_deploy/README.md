...
## Deploying a Model on A1 robot  <a name="realworld"></a>
...
The first step is to connect your development machine to the robot using ethernet. You should should modify the ip of your computer's wired connection to `192.168.123.50` and ping the robot to verify the connection: `ping 192.168.123.161` should return like `64 bytes from 192.168.123.161: icmp_seq=2 ttl=64 time=0.550 ms
`.
...
```
cd go2_gym_deploy/scripts && ./send_to_unitree.sh
```
...
```
chmod +x ~/go1_gym/go2_gym_deploy/installer/install_deployment_code.sh
cd ~/go1_gym/go2_gym_deploy/scripts
...
First:
```
cd ~/go1_gym/go2_gym_deploy/autostart
./start_unitree_sdk.sh
```

Second:
```
cd ~/go1_gym/go2_gym_deploy/docker
sudo make autostart
```
...
To deploy on the robot, replace the line https://github.com/Improbable-AI/walk-these-ways/blob/master/go2_gym_deploy/scripts/deploy_policy.py#L73 with `label = "$PDIR"`. Then re-run the `send_to_unitree.sh` script to update the files on the robot.
...
| ----------- | ----------- | ---------- |
| Out of disk space     | If you run out of disk space during `cd ~/go1_gym/go2_gym_deploy/installer && ./install_deployment_code.sh` consider changing the script to use `192.168.123.13` instead (at least in my Go1 Edu with 3 Jetson nano, I only had the required disk space to copy the tar and extract the image in only `192.168.123.13`). Alternatively, consider deploying on an external PC.       | https://github.com/Improbable-AI/walk-these-ways/issues/7 |
