    ...
    def calibrate(self, wait=True):
        ...
        for agent_name in self.agents.keys():
            if hasattr(self.agents[agent_name], "get_obs"):
                ...
                nominal_joint_pos = agent.initial_dof_pos
                ...
                for target in target_sequence:
                    action_scale=0.25
                    print("tar_pos:", -target + agent.default_dof_pos)
                    cal_action[:, 0:12] = (-target + agent.initial_dof_pos - agent.default_dof_pos)/action_scale
